...
struct TaPPStreamBuffer {
  char *buf;
  size_t cap;
  size_t pos = 0;

  // this struct always needs a buffer
  TaPPStreamBuffer() = delete;

  TaPPStreamBuffer(char *_buf, size_t _capacity) : buf(_buf), cap(_capacity) {}

  TaPPStreamBuffer &operator<<(const char *s) {
    if (!s || pos > cap - 1 || !buf)
      return *this;
  ...
  TaPPStreamBuffer &operator<<(unsigned long long n) {
    if (pos > cap - 1 || !buf)
      return *this;
  ...
  TaPPStreamBuffer &operator<<(int n) {
    if (pos > cap - 1 || !buf)
      return *this;
  ...
  TaPPStreamBuffer &fflush(FILE *out) {
    if (pos > cap - 1 || !buf)
      return *this;
